...
function gameBmulti_load()
	...
	-- First piece will be handled in startgame().
end
...
function gameBmulti_draw()
	...
	if gamestarted == false then
		local elapsed = love.timer.getTime() - starttimer
		if elapsed > 2 then
	...
	for i, v in pairs(tetribodiesp1) do
		...
		-- When failing, optionally tint with p1color.
	...
	if (not p1fail) and nextpiecep1 then
	...
	if (not p2fail) and nextpiecep2 then
	...
	if gamestate == "gameBmulti_results" then
		...
		if winner == 1 then
			-- Mario wins: draw appropriate sprites.
			if not jumpframe then
...
function gameBmulti_update(dt)
	...
	if not gamestarted then
		if newtime - starttimer > 3 then
			...
			startgame()
			gamestarted = true
		elseif newtime - starttimer > 2 and not beeped[3] then
			beeped[3] = true
			love.audio.stop(highscorebeep)
			love.audio.play(highscorebeep)
		elseif newtime - starttimer > 1 and not beeped[2] then
			beeped[2] = true
			love.audio.stop(highscorebeep)
			love.audio.play(highscorebeep)
		elseif newtime - starttimer > 0 and not beeped[1] then
			beeped[1] = true
			love.audio.stop(highscorebeep)
			love.audio.play(highscorebeep)
		end
...
function collideBmulti(a, b)
	-- Determine collision based on user data.
	if (a == "p1-" .. counterp1 and b ~= "p2-" .. counterp2) or (b == "p1-" .. counterp1 and a ~= "p2-" .. counterp2) then
		if not p1fail and a ~= "leftp1" and a ~= "rightp1" and b ~= "leftp1" and b ~= "rightp1" then
			endblockp1()
		end
	elseif (a == "p2-" .. counterp2 and b ~= "p1-" .. counterp1) or (b == "p2-" .. counterp2 and a ~= "p1-" .. counterp1) then
		if not p2fail and a ~= "leftp2" and a ~= "rightp2" and b ~= "leftp2" and b ~= "rightp2" then
			endblockp2()
		end
	elseif gamestate == "gameBmulti_results" then
...
function endblockp1()
	if gameno == 2 then
		for i, v in pairs(tetrishapesp1[counterp1]) do
			v:setMask(3, 2)
		end
	end
	if tetribodiesp1[counterp1]:getY() < losingY then
		p1fail = true
		if p2fail then endgame() end
	else
...
function endblockp2()
	if gameno == 2 then
		for i, v in pairs(tetrishapesp2[counterp2]) do
			v:setMask(2, 3)
		end
	end
	if tetribodiesp2[counterp2]:getY() < losingY then
		p2fail = true
		if p1fail then endgame() end
	else
		love.audio.stop(blockfall)
		love.audio.play(blockfall)
...
function endgame()
	...
	if p1wins > 99 then p1wins = p1wins % 100 end
	if p2wins > 99 then p2wins = p2wins % 100 end
end
