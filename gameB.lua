-- gameB.lua (Ported to LÖVE 11.5)
-- Handles the single-player “gameB” mode.
...
function gameB_load()
	...
	-- FIRST "nextpiece" --
	nextpiece = 1 -- math.random(7) can be re-enabled if desired.
	game_addTetriB()
	----------------
end
...
function createtetriB(i, uniqueid, x, y)
	...
	-- (Similar blocks for pieces J, L, O, S, T, Z follow.)
	end
end
...
function gameB_draw()
	...
	for i, v in pairs(tetribodies) do
		local x, y = v:getWorldCenter()
		love.graphics.point(x, y)
		for k, l in pairs(tetrishapes[i]) do
			local points = {l:getPoints()}
			for j = 1, #points, 2 do
				points[j] = points[j] + x
